...
int main() {
    ...
    uint64_t image1_value = ((uint64_t)160 << 44) |        // X=160 (bits 54-44)
    ...
    // This termination descriptor should be at the next 64-bit aligned address
    bram[2] = (uint32_t)(termination_value & 0xFFFFFFFF);    // Lower 32 bits for termination
    bram[3] = (uint32_t)(termination_value >> 32);          // Upper 32 bits for termination
